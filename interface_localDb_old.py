...
def _get_last_update_dates(conn, table_names):
    ...
    SELECT name, last_update, first_record_date 
    FROM ({query}) 
    GROUP BY name
    """, conn)
...
def _updateLookup_symbolRecords(conn, tablename, type, earliestTimestamp, numMissingDays = 5):
    ...
    ########### add max date as last updated date 
    ...
    ######### only update the last updated date if it is empty or < the max date of the current thing 
    else:
...
def getRecords(conn):
    ...
    try:
        print(' %s: Fetching records...'%(datetime.datetime.now().strftime("%H:%M:%S")))
    ...
    if not tableNames.empty:
        ...
        records['daysSinceLastUpdate'] = records['name'].map(num_days_since_last_update)

        records['interval'] = records.apply(lambda x: _addspace(x['interval']), axis=1)

    print(' %s: Records retrieval - Done!'%(datetime.datetime.now().strftime("%H:%M:%S")))
    return records
